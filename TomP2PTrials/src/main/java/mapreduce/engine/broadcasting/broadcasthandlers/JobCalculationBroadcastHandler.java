...
public class JobCalculationBroadcastHandler extends AbstractMapReduceBroadcastHandler {
	...
	public void evaluateReceivedMessage(IBCMessage bcMessage) {
		...
		synchronized (jobFuturesFor) {
			...
			if (job == null) {
				JobCalculationExecutor.dhtConnectionProvider().get(DomainProvider.JOB, jobId).addListener(new BaseFutureAdapter<FutureGet>() {
				...
				if (!bcMessage.outputDomain().executor().equals(JobCalculationExecutor.executorID())) {
					processMessage(bcMessage, job);
				}
			}
		}
