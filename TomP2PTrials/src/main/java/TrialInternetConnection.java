...
//		final String key = "Hello World";
		...
		if (peerID == 1) {
			...
			Peer myPeer = new PeerBuilder(Number160.createHash("client peer")).bindings(b).broadcastHandler(new MyBroadcastHandler())
			...
			if (action == "PUT") {
				...
				FuturePut putFuture = myPeerDHT.put(Number160.createHash(key)).data(new Data(value)).start();
